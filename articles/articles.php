...
if (isset($_GET['article_id']) && isnum($_GET['article_id'])) {
	...
	if ($rows) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<h4><a href='".BASEDIR."articles/articles.php?cat_id=".$data['article_cat_id']."'><strong>".$data['article_cat_name']."</a></strong> <span class='small2'>(".$data['article_count'].")</span></h4>";
		...
		echo "<div style='text-align:center'><br /> &nbsp;<img src='".THEME."images/icons/new_dl_icon.png' alt='Neu' title='Neu' style='border:0px; vertical-align:middle;' /><br /><br />\n</div>\n";
	}
	...
	if (dbrows($result) != 0) {
		...
		if (checkgroup($cdata['article_cat_access'])) {
			...
			if ($rows != 0) {
				...
				while ($data = dbarray($result)) {
					$class = ($i%2 ? "tbl1" : "tbl2");
					$new = ($data['article_datestamp']+604800 > time()+($settings['timeoffset']*3600)) ? "&nbsp;<label class='label label-success'> &nbsp;<img src='".THEME."images/icons/new_dl_icon.png' alt='Neu' title='Neu' style='border:0px; vertical-align:middle;' /></label>" : '';
					echo "<h4><strong><a href='".BASEDIR."articles/articles.php?article_id=".$data['article_id']."'>".$data['article_subject']."</a></strong>".$new."</h4>\n";
					echo preg_replace("/<!?--\s*pagebreak\s*-->/i", "", stripslashes($data['article_snippet']))."\n";
					$i++;
				}
				echo "<!--sub_article_cat-->";
				closetable();
				if ($rows > $settings['articles_per_page']) echo "<div align='center' style='margin-top:5px;'>\n".makepagenav($_GET['rowstart'], $settings['articles_per_page'], $rows, 3, BASEDIR."articles/articles.php?cat_id=".$_GET['cat_id']."&amp;")."\n</div>\n";
			} else {
