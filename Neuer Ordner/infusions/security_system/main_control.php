...
| Inhaber: Manuel Kurz                          |
| fixed version 10.02.2010, php 5.3 bugfix      |
+----------------------------------------------*/
...
if (isset($_COOKIE[COOKIE_PREFIX.'user'])) {
        $sec_cookie_vars = explode(".", htmlentities($_COOKIE[COOKIE_PREFIX.'user']));
        $sec_cookie_1 = isset($sec_cookie_vars[0]) && isNum($sec_cookie_vars[0]) ? $sec_cookie_vars[0] : "0";
        $sec_cookie_2 = isset($sec_cookie_vars[1]) && preg_match("/^[0-9a-z]{32}$/", $sec_cookie_vars[1]) ? $sec_cookie_vars[1] : "";
        $sec_cookie_3 = md5($sec_cookie_2);
        $sec_result = dbquery("SELECT * FROM ".DB_PREFIX."users WHERE user_id='".(int)$sec_cookie_1."' AND (user_password='".mysql_escape_string($sec_cookie_2)."' OR user_password='".$sec_cookie_3."')");
        unset($sec_cookie_vars,$sec_cookie_1,$sec_cookie_2,$sec_cookie_3);
        if (dbrows($sec_result) == 1) {
...
define("TRUE_REFERER",!isset($_SERVER['HTTP_REFERER']) || (isset($_SERVER['HTTP_REFERER']) && (!preg_match("/,http/i",$_SERVER['HTTP_REFERER']) || !preg_match("/,ftp/i",$_SERVER['HTTP_REFERER']))) ? TRUE : FALSE);
...
function cleantags($string) {
	...
	$string=preg_replace("[\[(.*?)\]]","", rawurldecode(urldecode($string)));
	$string=preg_replace("<\<(.*?)\>>","", rawurldecode(urldecode($string)));
	}
return $string;
}

$_SERVER['PHP_SELF']=cleantags($_SERVER['PHP_SELF']);
$_SERVER['QUERY_STRING']=!$_SERVER['QUERY_STRING'] ? "" : cleantags($_SERVER['QUERY_STRING']);

if ($sys_setting['license_accept']>0 && $sys_setting['secsys_started']==1) {
	
// function fixed in v 1.9.0
function secsys_hacker() {
global $sys_setting;
$hack = false;$bs_query_string = "";

$bs_search = array("?","(",")","[","]","<",">","{","}","'","\"","\\","$","\n","\r","%00","http://","https://","ftp","$","NULL","from%20".DB_PREFIX,"from ".DB_PREFIX,"from+".DB_PREFIX,"/*","*/");

$_SERVER['PHP_SELF'] = trim(htmlspecialchars(strip_tags(rawurldecode($_SERVER['PHP_SELF']))));
// Kontrolle auf SQL-Injektionen im Query-String
define("BS_QUERY",$_SERVER['QUERY_STRING']);

if (preg_match("[_(GET|REQUEST|SERVER|SESSION|ENV|POST|COOKIE)\[]",BS_QUERY)) {
	$hack=true;
} elseif (strip_tags(BS_QUERY)!=BS_QUERY) {$hack=true;} 
else {
if ((strlen($_SERVER['PHP_SELF'])-strrpos($_SERVER['PHP_SELF'],"/"))==1) {
	$hack=true;
	$_SERVER['QUERY_STRING']=$_SERVER['PHP_SELF'];
} else {
	// Kontrolle auf SQL-Injektionen durch $_GET
if (isset($_GET) && !empty($_GET) && is_array($_GET)) {
foreach ($_GET as $bs_key=>$bs_value) {
	if (str_replace($bs_search,"",$bs_key)!=$bs_key) {$hack=true;break;}
	$chkbsvalue = str_replace($bs_search,"",$bs_value);
	if ($chkbsvalue!=$bs_value) {$hack=true;break;}
		$bs_value = stripslashes(strip_tags(rawurldecode(urldecode($bs_value))));
		
	if ($bs_value!=str_replace("/","",$bs_value)) {$hack=true;break;}
		if (!empty($bs_value)) {
		$_GET[$bs_key] = mysql_escape_string(rawurldecode($bs_value));
		$_REQUEST[$bs_key] = mysql_escape_string(rawurldecode($bs_value));
		$bs_query_string.=($bs_query_string!="" ? "&" : "").$bs_key."=".$bs_value;
		}
} // Ende der Schleife
}}}
     ...
     $checker_agent=str_replace(explode(",",$agent),"*",strtolower($b_agent));
      ...
      if (strtolower($b_agent)!=strtolower($checker_agent)) {
    	$user_agent= (SYS_USER_AGENT!="" ? stripinput(str_replace('||', ' ', SYS_USER_AGENT)) : "");
          ...
          mysql_close();
	   redirect("http://sourceprotection.de/filterstop.html");exit;
	    }	
    }
    }
...
$spam_text="";
$spam_words=array();
$spam_checker=!empty($msg) && isset($_POST[$msg]) ? strtolower($_POST[$msg]) : "";
...
$spamstop=0;
     ...
     $message = $spam_checker=='' ? secsys_parser($spam_checker) : '';
     $message_org = $spam_checker;
	 $message_notags = cleantags($spam_checker); 
	 $fields_words = str_word_count(str_replace(array(".",":","_","!","?","\"","`","´","'",")","(","}","{","[","]","&",";",",",'/',"%","\\","§","#",'*','=','~','^','°'),' ',$message_notags));	
     
     if (!empty($message_notags) && $fields_words>=2) {
     	
     $spam_words=explode(",",$spamword);
	 	  
	 $check_spam1=str_replace($spam_words,"***SecSysSpam***",$message_org);
	 $check_spam2=str_replace($spam_words,"***SecSysSpam***",$message);	
	 $check_spam3=str_replace($spam_words,"***SecSysSpam***",$message_notags);
	 $found_spam=substr_count($check_spam1,'***SecSysSpam***');
	 $found_spam+=$found_spam>=2 ? 0 : substr_count($check_spam2,'***SecSysSpam***');
	 $found_spam+=$found_spam>=2 ? 0 : substr_count($check_spam3,'***SecSysSpam***');
     ...
     if ($found_spam>=2) {
		$user_agent= (SYS_USER_AGENT!="" ? stripinput(str_replace('||', ' ', SYS_USER_AGENT)) : "");
          ...
          $spam_msg=str_replace(explode(",",$spamword),explode(",",$spamword2),stripinput($spam_checker));
		...
		redirect(BASEDIR."setuser.php?logout=yes");exit;
		}
    }
          $result=dbquery("UPDATE ".DB_PREFIX."secsys_statistics SET spams=spams+1");
         mysql_close();
         redirect("http://sourceprotection.de/spamstop.html");exit;
	  }	  
...
global $sys_setting,$db_prefix,$locale;	
	...
	if (dbrows($check_blacklist)>0) {
		...
		mysql_close(); 
		redirect("http://sourceprotection.de/proxy_visit.html");exit;	
		} 
...
if ($sys_setting['proxy_visit']=="0") {
	if (!sec_proxyscan && (!iSYS_ADMIN || !iSYSMOD)) {	
...
mysql_close(); 
redirect(SEC_INFDIR."proxy.php");exit;
}
}

if ($sys_setting['proxy_register']==0) {
if(preg_match("/register.php/i",FUSION_SELF) && !sec_proxyscan()) {
...
if (!iSYS_ADMIN || !iSYSMOD) {
	...
	if ($bl_list>0) {
	redirect("http://sourceprotection.de/filterstop.html");
	} else {
...
if ($sys_setting['flood_active']>0)
 {
      ...
      if (preg_match("/\/forum\//i",FUSION_REQUEST)) {
        if (!checksysgroup($sys_setting['forum_access']) && !iSYSMOD) {
          ...
          {
          print_flood();
          redirect(INFUSIONS."security_system/restrict.php?check=forum");exit;
          }
        }
      }
      elseif (preg_match("/messages.php/i",FUSION_SELF)) {
        if (!checksysgroup($sys_setting['pm_access'])) {
          if (pm_control($sysdata['user_id'],$_POST['msg_send']))
          {
          print_flood();
          redirect(INFUSIONS."security_system/restrict.php?check=pm");exit;
          }
        }
        }
      elseif (preg_match("/contact.php/i",FUSION_SELF)) {
		...
		if (!isset($_POST['sendmessage']) || isset($_POST['sendmessage']) && $error!="") {
          ...
          {
          print_flood();
          redirect(INFUSIONS."security_system/restrict.php?check=contact");exit;
          }
		}
        }
      }
      ...
      if (isset($_POST['shout_message']))
      {
        ...
        if (!checksysgroup($sys_setting['shout_access'])) {
          ...
          {
          print_flood();
          redirect(INFUSIONS."security_system/restrict.php?check=shout");exit;
          }
        }
      }

      // Gästebuch
      if (preg_match("/guestbook.php/i",FUSION_SELF)) {
      if (isset($_POST['guest_message']))
      {
       if (!checksysgroup($sys_setting['gb_access'])) {
          ...
          {
          print_flood();
          redirect(INFUSIONS."security_system/restrict.php?check=guestbook");exit;
          }
        }
      }
      }
      ...
      if (isset($_POST['comment_message']))
      {
        if (!checksysgroup($sys_setting['comment_access'])) {
          ...
          print_flood();
          redirect(INFUSIONS."security_system/restrict.php?check=comments");exit;
          }
        }
      }
  }
