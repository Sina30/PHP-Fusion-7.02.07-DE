...
if (!isset($_GET['msg_send']) && !isset($_GET['msg_read']) && $_GET['folder'] != "options") {
	...
	if ($total_rows) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<td width='20%' class='tbl2' height='58'><a href='".FORUM."profile.php?lookup=".$data['user_id']."'>";
		...
		if (!$data['user_avatar']) { 
			...
			//echo "<br />";
			echo "<b>".profile_link($data['user_id'], $data['user_name'], $data['user_status'])."</td></b>\n";
